...
public class VideoGenerator(string ffmpegPath = "ffmpeg")
{
        ...
        int fps = 30)
    {
        ...
        Add("-vcodec", "libx264", "-preset", "ultrafast", "-crf", "24");
    ...
    /// subtitlePath may be absolute or relative (relative to output directory).
    /// </summary>
    public async Task GenerateFromClipsAsync(
        List<VideoSegment> segments,   // each must have ClipPath set
        string audioPath,
        string subtitlePath,           // full absolute path preferred on Windows
        string? musicPath,
        string outputPath,
        ...
        int fps = 30)
    {
        ...
        // backslashes → forward slashes, then colon after drive letter → \:
        var escapedSub = subtitlePath
            .Replace("\\", "/")
            .Replace(":", "\\:");

        // Windows fonts directory — libass needs this to resolve Arial on Azure Functions
        const string FontsDir = "C\\:/Windows/Fonts";
        ...
        // subtitles= filter (uses libass, same as ass= but with explicit fontsdir for Windows)
        var subFilter = $"subtitles='{escapedSub}':fontsdir='{FontsDir}'";

        if (clips.Count == 1)
        {
            // Single clip — no xfade needed
            f.Add($"[0:v]setsar=1,fps={fps}[vraw]");
            f.Add($"[vraw]{subFilter}[v]");
        }
        else
        {
    ...
    private static async Task RunAsync(ProcessStartInfo psi)
    {
        ...
        // Always log stderr — contains subtitle filter status even on success
