    ...
    string? deploymentName = null)
{
    ...
    private static string ExtractByRegex(string factText)
    {
        ...
        var model = brand != null ? SegmentPlanner.DetectModel(factText) : null;
        ...
        {
            parts.Add(brand);
            // Avoid repeating model if already embedded in brand (e.g. "Ford Model T")
            if (model != null && !brand.Contains(model, StringComparison.OrdinalIgnoreCase))
                parts.Add(model);
        }
        else
        {
